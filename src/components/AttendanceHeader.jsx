...
function CheckInWidget() {
  ...
  if (!context) {
    return <div className="flex w-full min-w-[220px] max-w-sm flex-col gap-3 rounded-lg border border-gray-800 bg-[#111827] p-4 lg:max-w-[260px] text-gray-400 text-sm">Loading...</div>;
  }

  const { attendance, checkIn, checkOut } = context
  
  if (!attendance) {
    return <div className="flex w-full min-w-[220px] max-w-sm flex-col gap-3 rounded-lg border border-gray-800 bg-[#111827] p-4 lg:max-w-[260px] text-gray-400 text-sm">No attendance data available</div>;
  }
  ...
  const sinceText =
    isIn && attendance?.checkedInAt
      ? `Since ${formatTime(attendance.checkedInAt)}`
      : null
...
export function AttendanceHeader({
  ...
  const safeDate = (selectedDate && !isNaN(new Date(selectedDate).getTime())) ? new Date(selectedDate) : new Date();
  const weekday = safeDate.toLocaleDateString('en-US', { weekday: 'long' })
  const dateLine = formatWireframeDate(safeDate)
