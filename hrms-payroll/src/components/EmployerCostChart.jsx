...
function ChartTooltip({ active, payload }) {
  ...
  return (
    <div className="rounded-lg border border-gray-700 bg-slate-900 px-4 py-2 text-sm shadow-lg">
      ...
      <p className="text-base text-white">{formatInr(row.cost)}</p>
    </div>
  )
}

export function EmployerCostChart({ monthlyData, yearlyData }) {
  ...
  return (
    <div className="flex h-full min-h-[360px] flex-col gap-6 rounded-2xl border border-gray-800 bg-[#0f172a] p-5 shadow-sm">
      <div className="flex items-center justify-between gap-4">
        <p className="text-sm text-gray-400">Employer cost</p>
        ...
        <ResponsiveContainer width="100%" height={280}>
          <BarChart data={data} margin={{ top: 8, right: 8, left: 0, bottom: 8 }}>
            ...
            <YAxis
              ...
              axisLine={false}
              tickLine={false}
              tickFormatter={(v) => (v >= 100000 ? `${(v / 100000).toFixed(1)}L` : String(v))}
            />
            <Tooltip content={<ChartTooltip />} cursor={{ fill: 'rgba(139, 92, 246, 0.08)' }} />
            <Bar dataKey="cost" fill="#7c3aed" radius={[8, 8, 0, 0]} maxBarSize={48} />
          </BarChart>
        </ResponsiveContainer>
      </div>
    </div>
